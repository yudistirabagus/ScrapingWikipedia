...
angular.module('wikiDiverApp').controller('AltCtrl', function ($scope, $http, $log, $timeout, $interval, $window) {
  var languages = {
    en: {
      name: 'english',
      seeAlso: ['See also'],
      stopWords: ['list of', 'index of', 'categories of', 'portal', 'disambiguation', 'outline of']
    }
  };
  $scope.supportedLanguages = Object.keys(languages).map(function (l) {
    return languages[l].name;
  }).sort().join(', ');
  $scope.stopWords = ['Wikipedia:', 'Category:', 'File:', 'Help:', 'Talk:', 'Template:', 'wikisource:', 'Commons:'];
  $scope.query = '';
  $scope.depth = 2;
  ...
  $scope.example = 'https://en.wikipedia.org/wiki/Microsoft_Windows';
  ...
  $scope.startCrawl = function () {
    ...
    $scope.started = true;
    $scope.doneParents = {};
    $scope.edgesIndex = {}; // Validate inputs before starting process

    if ($scope.validate()) {
      $scope.stopLayout();
      if ($scope.sigma) $scope.sigma.kill();
      $scope.network = new Graph({
        type: 'directed',
      ...
      }, 750); // Start crawl on pages from query
  ...
  $scope.initSigma = function () {
    ...
    if (!container) return;
    $scope.sigma = new Sigma.WebGLRenderer($scope.network, container); // Zoom buttons
    ...
    $scope.sigma.on('clickNode', function (e, a, b, c, d) {
      ...
      $timeout(function () {
        ...
        if (!e.captor.ctrlKey && !e.captor.metaKey) return $window.open(link, '_blank'); // add seed when double click ongraph nodes
    ...
    $timeout(function () {
      $scope.startLayout(true);
    }, 250);
  };

  $scope.validate = function () {
    if ($scope.alert && $scope.alert.indexOf('collecting') !== 0) $scope.alert = false;
    $scope.missingLang = false; // Check query

    if (!$scope.query.trim()) {
      checkParentsDepth();
      if (!$scope.alert) $scope.alert = 'please enter at least one wikipedia page';
      return false;
    }
    ...
    if (errors.length) $scope.alert = 'these are not valid wikipedia pages: ' + errors.join(' &nbsp;&nbsp;&nbsp; ');else if (langs.length > 1) $scope.alert = 'please enter wikipedia pages from the same language (you gave pages from ' + langs.join(', ') + ')';else if (!languages[lang]) {
      ...
      languages[lang].stopWords.forEach(function (s) {
        if (curSW.indexOf(s) === -1) $scope.stopWords.push({
          text: s
        });
      });
      return true;
  ...
  function checkParentsDepth() {
    if ($scope.getParents && $scope.depth > 2) $scope.alert = 'collecting parent links when crawling at a high depth can take a very long time';else $scope.alert = false;
  }
  ...
  function notFound(pageLink, updateResolved) {
    ...
    if (updateResolved) $scope.resolved++;else if ($scope.parentsPending) $scope.parentsPending--;
    if ($scope.running) $scope.running--;
  ...
  function parseAPISection(section, i, pageLink, updateResolved, callback, retries) {
    ...
    if (i) {
      if (updateResolved) $scope.pending++;else $scope.parentsPending++;
    }

    $http.jsonp('//' + $scope.lang + '.wikipedia.org/w/api.php?format=json&action=query&prop=revisions&titles=' + pageLink + '&rvprop=content&rvsection=' + section.index + '&redirects&callback=JSON_CALLBACK').success(function (linksData) {
      if (!linksData.query) return notFound(pageLink, updateResolved); // Collect links from the section content
  ...
  function downloadPageSeeAlsoLinks(pageLink, callback, updateResolved, retries) {
    if (retries === undefined) retries = 3; // Use existing cache

    var prefixedPage = ($scope.getAllLinks ? "ALL-" : "") + pageLink;

    if ($scope.cacheLinks[prefixedPage]) {
      if ($scope.cacheLinks[prefixedPage][0] === '#NOT-FOUND#') notFound(pageLink, updateResolved);else $timeout(function () {
        callback(filterStopWords($scope.cacheLinks[prefixedPage]));
      }, 10); // or find the page's SeeAlso section from API
    } else $http.jsonp('//' + $scope.lang + '.wikipedia.org/w/api.php?action=parse&page=' + pageLink + '&prop=sections&format=json&redirects&callback=JSON_CALLBACK').success(function (data) {
  ...
  function downloadPageAllLinks(pageLink, callback, updateResolved, retries) {
    if (retries === undefined) retries = 3;
    var cachePage = $scope.cacheLinks["ALL-" + pageLink]; // Use existing cache

    if (cachePage) {
      if (cachePage[0] === '#NOT-FOUND#') notFound(pageLink, updateResolved);else $timeout(function () {
        callback(filterStopWords(cachePage));
      }, 10); // or find the page's SeeAlso section from API
    } else $http.jsonp('//' + $scope.lang + '.wikipedia.org/w/api.php?action=parse&page=' + pageLink + '&prop=links&format=json&redirects&callback=JSON_CALLBACK').success(function (data) {
      if (!data.parse) return notFound(pageLink, updateResolved);
      var links = data.parse.links.map(function (l) {
        return l['*'];
      });
  ...
  function addNode(pageName, level, seed, parentNode) {
    ...
    if ($scope.network.order == 1000) $scope.startLayout();
  ...
  function getSons(page, ind) {
    ($scope.getAllLinks ? downloadPageAllLinks : downloadPageSeeAlsoLinks)(page, function (links) {
      links.forEach(function (d) {
        ...
        if (ind + 1 < $scope.depth) getRelatives(d, ind + 1);
      });
      $scope.resolved++;
      if ($scope.running) $scope.running--;
    }, true);
  ...
  function getParents(page, ind, seed) {
    ...
    $scope.doneParents[page] = true; // Get backlinks to the page from the API

    $http.jsonp('//' + $scope.lang + '.wikipedia.org/w/api.php?action=query&bltitle=' + page + '&blnamespace=0&list=backlinks&blredirect&blfilterredir=nonredirects&bllimit=250&format=json&callback=JSON_CALLBACK').success(function (data) {
      ...
      filterStopWords(data.query.backlinks.map(function (l) {
        ...
        if ($scope.cacheLinks[prefixedParent]) {
          if ($scope.cacheLinks[prefixedParent][0] !== '#NOT-FOUND#') validateParentFromLinks(page, parentLink, ind, $scope.cacheLinks[prefixedParent]);
          if ($scope.parentsPending) $scope.parentsPending--;
        } else {
  ...
  $scope.downloadCSV = function () {
    ...
    var blob = new Blob([csvtxt], {
      type: 'data:text/csv;charset=utf-8'
    });
    saveAs(blob, 'crawldata.tsv');
  };

  $scope.downloadGEXF = function () {
    if ($scope.network) {
      var blob = new Blob([gexf.write($scope.network, {
        formatNode: function formatNode(key, attributes) {
          return {
  ...
  $scope.clearQueue = function () {
    ...
    $scope.queue.forEach(function (d) {
      if (d.type === 'sons') $scope.pending--;else if ($scope.parentsPending) $scope.parentsPending--;
    });
    $scope.queue = [];
  };
  ...
  $scope.$watch($scope.working, function (n, o) {
    if (!n && n !== o) $timeout(function () {
      if ($scope.working() || !$scope.layout) return;
      if (!Object.keys($scope.edgesIndex).length) $('#warning').append('<div class="col-md-12"><div class="alert alert-danger">Warning: no result found from these seeds.</div></div>');
      $scope.startLayout();
